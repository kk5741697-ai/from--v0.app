...
export function UnifiedToolLayout({
  ...
  const [showToolsInterface, setShowToolsInterface] = useState(!showUploadArea)
  ...
  const handleFileUpload = async (uploadedFiles: FileList | null) => {
    ...
    const maxSafeSize = 50 * 1024 * 1024 // 50MB total limit for stability

    if (totalSize > maxSafeSize) {
      toast({
        title: "Files too large",
        description: `Total size ${Math.round(totalSize / (1024 * 1024))}MB exceeds 50MB limit for stability`,
        variant: "destructive"
      })
      return
    }
  ...
  const getPDFInfo = async (file: File): Promise<{ pageCount: number; pages: any[] }> => {
    // Simplified PDF info for demo - in production use PDF.js
  ...
  const createPDFThumbnail = (pageNum: number, totalPages: number): string => {
    ...
    ctx.fillText(`of ${totalPages}`, canvas.width / 2, canvas.height / 2 + 15)
  ...
  const resetTool = () => {
    ...
    setShowToolsInterface(toolType === "qr")
  ...
  // Upload Interface
  if (!showToolsInterface && showUploadArea) {
    return (
            ...
            <div className="max-w-2xl mx-auto">
              ...
              >
                ...
                <h3 className="text-2xl font-semibold mb-3 text-gray-700 group-hover:text-blue-600 transition-colors">
                  Drop {toolType === "pdf" ? "PDF" : toolType === "image" ? "images" : "files"} here
                </h3>
                ...
                <div className="mt-6 space-y-2 text-center">
                  ...
                  <p className="text-xs text-gray-400">Up to {maxFiles} files • Advanced processing • Secure & private</p>
                </div>
              </div>
            </div>
          </div>
  ...
  return (
    <div className="tool-wrapper min-h-screen bg-background">
            ...
            <>
                      ...
                      <CardContent className="p-3">
                        <div className="relative">
                          ...
                          {title.includes("Crop") && (
                            ...
                            >
                              <div className="absolute -top-1 -left-1 w-3 h-3 bg-cyan-500 border-2 border-white rounded-full cursor-nw-resize"></div>
                              <div className="absolute -top-1 -right-1 w-3 h-3 bg-cyan-500 border-2 border-white rounded-full cursor-ne-resize"></div>
                              <div className="absolute -bottom-1 -left-1 w-3 h-3 bg-cyan-500 border-2 border-white rounded-full cursor-sw-resize"></div>
                              <div className="absolute -bottom-1 -right-1 w-3 h-3 bg-cyan-500 border-2 border-white rounded-full cursor-se-resize"></div>
                            </div>
                          )}
                        </div>
              ...
              {toolType === "qr" && (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-600">QR code preview will appear here</p>
                </div>
              )}
            </>
          )}
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="canvas flex-1 overflow-hidden bg-gray-50">
            <ScrollArea className="h-full">
              <div className="p-6 space-y-4" ref={canvasRef}>
                {children || (
                  <>
                    {toolType === "image" && files.length > 0 && (
                          ...
                          <Card key={file.id} className="relative group">
                            <CardContent className="p-3">
                              <div className="relative">
                                ...
                                {title.includes("Crop") && (
                                  <div 
                                    className="absolute border-2 border-cyan-500 bg-cyan-500/10 cursor-move crop-area"
                                    style={{
                                      left: `${cropArea.x}%`,
                                      top: `${cropArea.y}%`,
                                      width: `${cropArea.width}%`,
                                      ...
                                      return (
                                        <div
                                          ...
                                          className={`pdf-page-thumbnail relative cursor-pointer border-2 rounded-lg overflow-hidden transition-all ${
                                            isSelected ? 'selected border-red-500 ring-2 ring-red-200' : 'border-gray-200 hover:border-red-300'
                                          }`}
                                          onClick={() => togglePageSelection(pageKey)}
                                        >
                                          <img
                    ...
                    {toolType === "qr" && (
                      <div className="text-center py-12">
                        <div className="qr-preview max-w-sm mx-auto">
      ...
      <div className="hidden lg:block">
        <div className="fixed right-0 bottom-0 w-80 xl:w-96 p-6 border-t bg-gray-50 space-y-3">
          {isProcessing && (
